...
export const LEADER_CONDUCTOR_ROLE_ROUTING_DEGRADE_BLOCK = [
  ...
  'Before any Ralplan-originated action that requires adapted role authority, run `omx ralplan preflight --json` and stop on `unsupported_documented_leader_proof`.',
  'Do not apply that gate to unrelated guarded install/sync, status/health checks, or runtime work; those flows retain their own safety gates.',
